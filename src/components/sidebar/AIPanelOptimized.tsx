...
import { AIService, AIRequest, AI_PROVIDERS } from '../../services/aiProviders';
...
const ConfigurationPrompt = memo(({ onShowSettings }: { onShowSettings: () => void }) => (
  <div className="p-4">
...
ConfigurationPrompt.displayName = 'ConfigurationPrompt';

const ProviderStatus = memo(({ 
  activeProvider, 
  providerConfig, 
  settings 
}: {
...
const QuickPromptButton = memo(({ 
  ...
  <button
    onClick={() => onGenerate(prompt, categoryTitle.toLowerCase())}
...
export const AIPanelOptimized = memo(({ aiService, onShowSettings }: AIPanelProps) => {
  ...
  const settings = useMemo(() => aiService.getSettings(), [aiService]);
  const activeProvider = useMemo(() => 
    AI_PROVIDERS.find(p => p.id === settings.defaultProvider), 
  ...
  const insertIntoEditor = useCallback((content: string) => {
  ...
  if (!isProviderConfigured) {
    return <ConfigurationPrompt onShowSettings={onShowSettings} />;
  }
...
AIPanelOptimized.displayName = 'AIPanelOptimized';